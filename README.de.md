...
- **Linux**: das tatsächliche *Anwenden* eines Selbst-Updates (`dpkg -i`/`rpm -U`), das auf einem echten Host vollständig durchläuft — ein *separater* Mechanismus vom oben verifizierten Paketierungs-Installations-/Upgrade-Pfad (`LinuxPackageApplier`s eigener, in `systemd-run` eingebetteter Aufruf, nicht ein einfaches `dpkg -i`/`rpm -i`) — der zugrunde liegende Cgroup-Escape-Mechanismus wurde live bestätigt, ein vollständiger Selbst-Update-Zyklus danach aber nicht erneut verifiziert, und [updatewatch2-agent#24](https://github.com/vulture20/updatewatch2-agent/issues/24) dokumentiert einen echten Bericht, der auf ein mögliches stilles Fehlschlagen hindeutet.
- **Paketierung**: konkret die `systemctl enable`/`start`/`stop`-Zweige der Paketierungsskripte (`postinst.sh`/`prerm.sh`/`postrm.sh`) — jeder bisherige Installations-/Upgrade-/Deinstallationstest lief gegen ein minimales Container-Image ganz ohne `systemctl`-Binary; der echte Produktions-Host `hpn54l` (nur `.deb`) bleibt der einzige Ort, an dem dieser Zweig je real durchlaufen ist. Die arm64-Bestätigung oben lief unter QEMU-Emulation, nicht auf echter arm64-Hardware — das Multi-Arch-Docker-Image (ein separates, serverseitiges Thema) wurde ebenso nie auf echter arm64-Hardware gezogen/ausgeführt. Windows-on-ARM (`win-arm64`) bleibt vollständig unbestätigt auf einem echten Gerät — ein Windows-Emulationsweg wurde nicht versucht.
