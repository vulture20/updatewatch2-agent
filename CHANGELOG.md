...
- **An admin can now remotely restart the agent's own service process from the admin UI, at the user's explicit request ("Es fehlt noch die Funktion um den Client über die Oberfläche neu starten zu können") — protocol `0.11.0`.** A distinct, new agent-lifecycle action, never to be confused with an OS reboot or with an OS-update install (CLAUDE.md's "update installation never triggers a reboot itself" rule is unrelated — this restarts only this agent's own service, nothing else). Delivered exactly like a remote install trigger: the server surfaces a pending restart request as an additive `restartRequested` field on the existing `alive` heartbeat response, `HeartbeatWorker` reacts to it inline on that same tick (last, deliberately, after everything else the tick would otherwise do — install, self-update, cert renewal — since a successful restart ends this very process shortly after), and a new `POST .../restart-ack` call acknowledges the outcome. New `Restart/IAgentRestarter` interface, `Restart/Windows/WindowsAgentRestarter` and `Restart/Linux/LinuxAgentRestarter` implementations, chosen in `Program.cs` the same way every other platform seam already is.
- **`IAgentRestarter.RequestRestart()` only ever launches the platform's stop-then-start mechanism and returns almost immediately — it never waits for it to finish, since finishing means this very process is gone.** This is what makes it safe for `HeartbeatWorker` to still reach the server with `restart-ack` in the brief window between launching the restart and the process actually going down; if that ack itself fails to land, the server simply reports the same restart as still pending, and the freshly-restarted process's own first heartbeat sees it again — one harmless extra restart cycle, not a stuck state.
- **Windows**: `WindowsAgentRestarter` writes a tiny self-deleting `.bat` script to the temp directory (`sc stop "UpdateWatch2 Agent"` then, after a short delay, `sc start "UpdateWatch2 Agent"`, then deletes itself) and launches it as a detached `cmd.exe` child — a plain `sc.exe` call from this process directly can't work, since stopping this very service via SCM would terminate this process before it could ever run the follow-up `start` command itself. The script content is pulled out as a separate, testable `BuildRestartScript` static method (mirroring this codebase's `BuildInstallArgs`/`BuildInstallCommand` convention) specifically so the actual command sequence has real unit test coverage even though launching it does not. **Not live-verified** — same standing honesty caveat as every other Windows-only class in this codebase (no Windows host available); re-verify a real trigger-restart cycle on an actual Windows service before relying on this in production.
- **Linux**: `LinuxAgentRestarter` runs a plain `systemctl restart --no-block updatewatch2-agent` child process — deliberately NOT wrapped in the detached `systemd-run --scope` cgroup-escape `SelfUpdate/Linux/LinuxPackageApplier`'s own dpkg/rpm install step needs. The two situations are genuinely different, not an oversight: `dpkg`/`rpm` themselves have to keep running, mid-transaction, all the way through this unit being stopped and restarted, and get killed by the same SIGTERM-to-cgroup event that stops the unit — but a bare `systemctl restart` is only a thin client that hands one job ("stop then start this unit") to systemd's PID 1 manager over D-Bus and returns; PID 1 isn't part of this unit's cgroup and carries out the actual stop-then-start entirely on its own, unaffected by this process (or the `systemctl` child) being torn down moments later. `--no-block` just avoids the `systemctl` child hanging around waiting for a "job complete" reply it may never get to see. **Live-verified for real, not just reasoned about** — a throwaway harness on a real systemd host (`hpn54l`, systemd 257) had a transient unit's own process call `systemctl restart --no-block` on itself, and confirmed a fresh instance of the unit actually came up afterward (repeatedly, since the harness script re-triggered its own restart with no delay, until systemd's own start-limit-burst protection stopped it after 5 cycles) — exactly the mechanism this class's production use depends on.
- Real unit test coverage: `WindowsAgentRestarterTests`/`LinuxAgentRestarterTests` (the pure command/script-building halves) and new `HeartbeatWorker` tests (`WorkerTests`) covering the trigger-and-acknowledge-success path, the no-request no-op path, and the trigger-throws-and-acknowledges-failure path — mirroring the existing install-trigger test coverage exactly.
