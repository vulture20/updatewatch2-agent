...
- **The hostname this agent reports to — and is identified by on — the server is now overridable via a local config/registry parameter, at the user's explicit request ("Der an den Server gemeldete und damit auch genutzte Hostname des Agents sollte über einen Parameter in der Config-Datei bzw. der Registry überschreibbar sein.").** New `AgentOptions.HostnameOverride` (nullable, unset by default — today's existing OS-reported-hostname behavior is unchanged): `ServerClient`'s six route-building call sites (`register`/`alive`/`report-updates`/`install-ack`/`reboot-ack`/`renew`) now go through the new `AgentOptions.ResolveHostname()` (the override when non-blank, otherwise `Environment.MachineName`, the single prior behavior) instead of calling `Environment.MachineName` inline. Deliberately does **not** affect the separately self-reported `DnsName` metadata field (`ResolveDnsName()`, purely informational, shown in the admin UI) — confirmed as the intended scope via an explicit clarifying question before implementing, since the two are genuinely different concepts (identity/routing vs. self-reported metadata). Local-only, never pushed by the server, same discipline as `AllowUnauthenticatedPackages`. No installer (NSIS) support added — manual registry/config edit only, matching that same precedent, per an explicit clarifying question answered before implementing.
  - **A real, deliberate footgun, surfaced rather than silently handled, per an explicit clarifying question answered before implementing**: the server's `alive`/`renew`/`reboot-ack` endpoints all reject a request whose URL hostname doesn't match the identity baked into the presented client certificate's Subject at issuance — so changing this override on an agent that already holds a certificate would otherwise make every authenticated call fail. `HeartbeatWorker` now detects this *locally* every tick, before ever attempting a call — comparing the loaded certificate's own Subject (via `X509Certificate2.GetNameInfo(X509NameType.SimpleName, ...)`) against `ResolveHostname()`'s current result — rather than reacting to the resulting 401/403, which is indistinguishable at the HTTP level from a genuine certificate rejection and would otherwise silently trip this agent's own self-heal mechanism (dropping the certificate and re-registering under the new hostname, leaving the old `Agent` row orphaned server-side with no admin involvement at all). On a mismatch, a clear warning is logged every tick it persists (matching `AllowUnauthenticatedPackages`' own "warn on every relevant call while the condition holds" convention) and the heartbeat/renewal calls are skipped entirely for that tick — deliberately *not* acted on automatically. An admin who wants to actually rename an already-onboarded agent must delete its old server-side entry and remove this agent's local client certificate by hand, which lets `RegistrationWorker`'s existing lost-certificate recovery path register it fresh under the new name.
  - `WindowsRegistryConfigStore`/`LinuxFileConfigStore` both updated (Linux needed no code change — plain JSON serialization already round-trips a new nullable field automatically); `installer/linux/postinst.sh`'s starter config and both READMEs' config tables updated to match.

## [1.0.22] - 2026-09-18

### Changed

- **`Microsoft.Extensions.Http`'s own automatic HTTP-client logging on the shared `IServerClient` is now Debug-only, at the user's explicit request after reviewing `log-level-audit.md`.** `AddHttpClient<IServerClient, ServerClient>` wraps the shared handler in two library-provided logging handlers whose "Start/End processing HTTP request", "Sending HTTP request"/"Received HTTP response headers", and — on a genuine transport failure — "HTTP request failed" (full exception/stack trace) messages are hardcoded at Information severity, duplicating `ServerClient`'s own explicit `LogDebug` lines for the same `alive`/`report-updates`/`install-ack`/`reboot-ack`/`renew`/CA-bundle-fetch calls, already visible at this agent's default log level. `Program.cs` now registers two additional category-specific `builder.Logging.AddFilter(...)` calls, right next to the existing generic one, targeting `System.Net.Http.HttpClient.IServerClient.{LogicalHandler,ClientHandler}`: each reads the same mutable `logLevelState.Current` and only lets the category through at its normal Debug threshold when that value is genuinely Debug, raising it to Warning (a complete mute — neither category ever logs above Information) at every other setting. Since this reuses the exact delegate-based mechanism `logLevelState`/`ILogLevelState` already provides for a server-pushed LogLevel change, it needs no extra plumbing and takes effect the moment `HeartbeatWorker.ApplyPushedSettings` calls `logLevelState.Update(...)` — no restart. A fifth message in this category, "HTTP request failed after {ElapsedMilliseconds}ms" (EventId 104, also Information, fires only on a real connect/transport failure), was found only while implementing this and demoted alongside the other four at the user's choice, since this agent's own explicit Warning logging (the retry-warning in `ServerClient`, `HeartbeatWorker`'s "Failed to send alive heartbeat") already reports the same failure more concisely. Live-verified against a throwaway harness reproducing the exact filter expressions, with a real `AddHttpClient` client pointed at a deliberately unreachable host: at "Information", none of the seven category lines (100/100/101/101/104/104 plus two Trace-only header dumps that were never reachable anyway) appear, even on a genuine connection-refused error with a full stack trace; at "Debug", all appear exactly as before. See the server repo's `log-level-audit.md` (rows 230–235, 246–247) for the full audit this originated from — the sibling change, server v1.3.26, does the equivalent for the server's own `IGitHubReleaseClient`.
