...
- **Linux**: actually applying a self-update (`dpkg -i`/`rpm -U`) completing end to end on a real host — a *separate* mechanism from the packaging install/upgrade path above (`LinuxPackageApplier`'s own `systemd-run`-wrapped invocation, not a plain `dpkg -i`/`rpm -i`) — the cgroup-escape mechanism it depends on was confirmed live, but a full self-update cycle after that fix was not re-verified, and [updatewatch2-agent#24](https://github.com/vulture20/updatewatch2-agent/issues/24) tracks a real report suggesting it may still fail silently.
- **Packaging**: the packaging scripts' (`postinst.sh`/`prerm.sh`/`postrm.sh`) `systemctl enable`/`start`/`stop` branches specifically — every install/upgrade/uninstall test so far ran against a minimal container image with no `systemctl` binary at all; the real production `hpn54l` host (`.deb` only) remains the sole place that branch has ever actually run for real. The arm64 packaging confirmation above was under QEMU emulation, not real arm64 hardware — the multi-arch Docker image (a separate, server-side concern) likewise has not been pulled/run on real arm64 hardware. Windows-on-ARM (`win-arm64`) remains entirely unconfirmed on a real device — no Windows emulation path was attempted.
