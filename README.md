...
> ✅ **Stable.** The certificate-based onboarding and heartbeat are implemented and tested end to end against a real running server; self-update's negotiation half (offer, download, SHA-256 verification) is too. The real Windows Update API (WUApiLib) integration, the Windows installer's install/uninstall behavior, Windows self-update's apply step, remote reboot scheduling on both platforms, and Windows Event Log output have all since been confirmed against real hosts, including a selective (per-update) install. The Linux `dnf`/`yum` update path (search, download-only, selective and full install, reboot-required detection) has since been confirmed against real Fedora (dnf5) and Rocky Linux (classic dnf4) hosts too, plus bare legacy `yum` at the CLI level (CentOS 7). Linux self-update's apply step has **not** been verified against a real target host yet. The Linux arm64 packaging install/upgrade path (`dpkg`/`rpm`) has since been run for real too, though only under QEMU emulation, not real arm64 hardware. The Windows-on-ARM install path remains entirely unverified — no real device has ever been available. See [Project status](#-project-status) below.
...
UpdateWatch2 was built with **vibe coding**: implemented and iterated on with [Claude Code](https://claude.com/claude-code) (Anthropic) in conversation, rather than hand-written line by line, driven by a human-authored architecture brief. The certificate lifecycle (registration, approval, renewal, re-issuance, CA root rotation) and the alive heartbeat have been run live against a real server, and both the Linux `apt` and `dnf`/`yum` update-*detection*-and-*install* paths have been run live — `apt` against a real package cache, `dnf`/`yum` end to end against real Fedora (dnf5) and Rocky Linux (classic dnf4) hosts (search, download-only, selective and full install, reboot-required detection, via the throwaway container `scripts/run-fedora-test-server.sh` stands up — Rocky is a permanent second leg in CI's own matrix alongside Fedora), plus legacy bare `yum` (CentOS 7) confirmed at the CLI level; all are covered by an automated (xUnit) test suite. The Windows Update API (WUApiLib COM) integration has since been run live end to end against a real Windows host — search, download, install (including a selective, per-update install), and reboot detection all confirmed working — and the NSIS installer's install/uninstall behavior actually run through `sc.exe` has likewise been confirmed on a real Windows host, closing [updatewatch2-agent#13](https://github.com/vulture20/updatewatch2-agent/issues/13). Agent self-update's *negotiation* half (the server offering a release, this agent downloading it and verifying its SHA-256) has been run live too, and — on Windows — so has actually *applying* one: `WindowsInstallerApplier` silently re-running the NSIS installer, including the service delete/recreate sequence, has been confirmed working end to end against a real Windows host. `shutdown.exe`-based remote reboot scheduling and Windows Event Log output have likewise been confirmed on a real Windows host, and Linux's own `systemd-run --on-active=... -- systemctl reboot` remote reboot has been confirmed against a real Linux host too — both platforms' reboot scheduling is now live-verified. A number of pieces are explicitly **not yet live-verified against a real target host**, called out as such in code comments:
...
- **Linux**: actually applying a self-update (`dpkg -i`/`rpm -U`) completing end to end on a real host — a *separate* mechanism from the OS-update install path above (`LinuxPackageApplier`, not `DnfUpdateSession`/`AptUpdateSession`) — the cgroup-escape mechanism it depends on was confirmed live, but a full self-update cycle after that fix was not re-verified, and [updatewatch2-agent#24](https://github.com/vulture20/updatewatch2-agent/issues/24) tracks a real report suggesting it may still fail silently.
- **Packaging**: `.rpm` install/upgrade on x86_64 (structural inspection only, never run through a real package manager). arm64 is a partial exception, and more so than before: each release's CI pipeline already starts the published `linux-arm64` binary on a native (not emulated) arm64 runner before packaging, live-verifying real startup/registration code on real arm64 hardware; separately (agent v1.0.30), a real `dpkg -i`/`dpkg -i` upgrade/`dpkg -r` and `rpm -i`/`rpm -U --force`/`rpm -e` cycle — including config-preservation across an upgrade — was run for real against real arm64 `.deb`/`.rpm` packages, but under **QEMU emulation** (`--platform linux/arm64` via `tonistiigi/binfmt`), not real arm64 hardware, and without `systemd` running as PID 1 in either test image, so the packaging scripts' `systemctl enable`/`start`/`stop` branches specifically were not exercised. Windows-on-ARM (`win-arm64`) remains entirely unconfirmed on a real device — no Windows emulation path was attempted.

Treat the pieces called out above as well-researched but not yet confirmed on a real target host — everything else, including the Windows Update API integration, the installer's install/uninstall behavior, the Windows self-update apply step, remote reboot on both platforms, and Windows Event Log output above, has been live-verified end to end.
