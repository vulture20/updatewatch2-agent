...
public class RegistrationWorkerTests : IDisposable
{
    ...
    private class FakeServerClient(byte[]? caBytes = null, Func<string?, RegisterResult>? onRegister = null) : IServerClient
    {
        ...
        public Task<AliveResult> SendAliveAsync(bool? rebootRequired = null, CancellationToken ct = default) => Task.FromResult(AliveResult.From(AliveOutcome.Success));
        ...
        public Task AcknowledgeRebootAsync(RebootOutcome outcome, string? errorDetail, CancellationToken ct = default) => Task.CompletedTask;
